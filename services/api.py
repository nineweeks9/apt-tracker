...
async def fetch_month(client, sigungu_code, year_month):
    params = {"serviceKey": PUBLIC_DATA_API_KEY, "LAWD_CD": sigungu_code, "DEAL_YMD": year_month, "pageNo": "1", "numOfRows": "9999"}
    try:
        resp = await client.get(APT_TRADE_URL, params=params, timeout=30.0)
        if resp.status_code != 200: return []
        ct = resp.headers.get("content-type", "")
        if "json" in ct: return _parse_json(resp.json())
    ...
    try:
        ...
        for item in item_list:
            try:
                ...
                results.append({"apt_name": str(item.get("aptNm", item.get("아파트", ""))).strip(), "dong": str(item.get("umdNm", item.get("법정동", ""))).strip(), "deal_amount": amt, "area": float(item.get("excluUseAr", item.get("전용면적", 0))), "floor": _si(item.get("floor", item.get("층"))), "build_year": _si(item.get("buildYear", item.get("건축년도"))), "deal_year": _si(item.get("dealYear", item.get("년"))), "deal_month": _si(item.get("dealMonth", item.get("월"))), "deal_day": _si(item.get("dealDay", item.get("일")))})
            except: continue
        return results
    except: return []
    ...
    try:
        ...
        for item in root.findall(".//item"):
            try:
                ...
                results.append({"apt_name": (item.findtext("아파트") or "").strip(), "dong": (item.findtext("법정동") or "").strip(), "deal_amount": amt, "area": float(item.findtext("전용면적") or 0), "floor": _si(item.findtext("층")), "build_year": _si(item.findtext("건축년도")), "deal_year": _si(item.findtext("년")), "deal_month": _si(item.findtext("월")), "deal_day": _si(item.findtext("일"))})
            except: continue
        return results
    except Exception as e:
        ...
        for i in range(0, len(months), 5):
            ...
            for month_data in results:
                for tx in month_data:
                    if tx["apt_name"] == apt_name: all_tx.append(tx)
            if i + 5 < len(months): await asyncio.sleep(0.3)
